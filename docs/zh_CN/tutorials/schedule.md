...
可以创建一个名为 `rsicls/core/optimizer` 的文件夹，并在目录下的一个文件，如 `rsicls/core/optimizer/my_optimizer.py` 中实现该自定义优化器：
...
- 修改 `rsicls/core/optimizer/__init__.py`，将其导入至 `optimizer` 包；再修改 `rsicls/core/__init__.py` 以导入 `optimizer` 包

  创建 `rsicls/core/optimizer/__init__.py` 文件。
  新定义的模块应导入到 `rsicls/core/optimizer/__init__.py` 中，以便注册器能找到新模块并将其添加：

```python
# 在 rsicls/core/optimizer/__init__.py 中
...
`rsicls.core.optimizer.my_optimizer` 模块将会在程序开始阶段被导入，`MyOptimizer` 类会随之自动被注册。
注意，只有包含 `MyOptmizer` 类的包会被导入。`rsicls.core.optimizer.my_optimizer.MyOptimizer` **不会** 被直接导入。

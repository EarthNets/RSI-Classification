...
from rsicls.models import (MobileNetV2, MobileNetV3, RegNet, ResNeSt, ResNet,
                          ResNeXt, SEResNet, SEResNeXt, SwinTransformer,
                          TIMMBackbone)
from rsicls.models.backbones.timm_backbone import timm

backbone_configs = dict(
    mobilenetv2=dict(
        backbone=dict(
            type='rsicls.MobileNetV2',
            widen_factor=1.0,
    ...
    mobilenetv3=dict(
        backbone=dict(
            type='rsicls.MobileNetV3',
            norm_cfg=dict(type='GN', num_groups=2, requires_grad=True),
            out_indices=range(7, 12)),
        out_channels=[48, 48, 96, 96, 96]),
    regnet=dict(
        backbone=dict(type='rsicls.RegNet', arch='regnetx_400mf'),
        out_channels=384),
    resnext=dict(
        backbone=dict(
            type='rsicls.ResNeXt', depth=50, groups=32, width_per_group=4),
        out_channels=2048),
    resnet=dict(
        backbone=dict(type='rsicls.ResNet', depth=50), out_channels=2048),
    seresnet=dict(
        backbone=dict(type='rsicls.SEResNet', depth=50), out_channels=2048),
    seresnext=dict(
        backbone=dict(
            type='rsicls.SEResNeXt', depth=50, groups=32, width_per_group=4),
        out_channels=2048),
    resnest=dict(
        backbone=dict(
            type='rsicls.ResNeSt',

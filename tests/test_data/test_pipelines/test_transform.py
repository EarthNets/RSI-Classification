...
import rsicls.datasets.pipelines.transforms as rsicls_transforms
from rsicls.datasets.builder import PIPELINES
from rsicls.datasets.pipelines import Compose
...
def test_randomcrop():
    ...
    np.random.seed(seed)
    aug = []
    aug.extend([rsicls_transforms.RandomCrop(**kwargs)])
    composed_transform = Compose(aug)
...
def test_randomresizedcrop():
    ...
    with pytest.raises(ValueError):
        ...
        aug = []
        aug.extend([rsicls_transforms.RandomResizedCrop(**kwargs)])
        composed_transform = Compose(aug)
        results = dict()
        results['img'] = ori_img
    ...
    np.random.seed(seed)
    aug = []
    aug.extend([rsicls_transforms.CenterCrop(**kwargs)])
    aug.extend([rsicls_transforms.Resize(**resize_kwargs)])
    composed_transform = Compose(aug)
    results = dict()
    results['img'] = ori_img
...
def test_randomgrayscale():
    ...
    aug = []
    aug.extend([rsicls_transforms.RandomGrayscale(**kwargs)])
    composed_transform = Compose(aug)
    print(composed_transform)
    results = dict()
    results['img'] = in_img

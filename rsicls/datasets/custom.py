...
class EODataset(BaseDataset):
    ...
    Args:
        data_root (str): The path of data directory.
        pipeline (Sequence[dict]): A list of dict, where each element
            represents a operation defined in :mod:`rsicls.datasets.pipelines`.
            Defaults to an empty tuple.
        classes (str | Sequence[str], optional): Specify names of classes.
    ...
    def load_annotations(self):
        ...
        for data_item in self._dataset:
            info = {'img_prefix': self.data_prefix}
            info['img_info'] = {'filename': data_item['filename']}
            info['gt_label'] = np.array(data_item['cls_idx'], dtype=np.int64)
            data_infos.append(info)
        return data_infos
